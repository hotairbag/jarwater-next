...
export default async function ExplainersPage({ params }: Props) {
  ...
  const serviceSchema = getServiceSchema(
    tMeta("explainersTitle"),
    tMeta("explainersDescription"),
    `${baseUrl}services/explainers`
  );

  const breadcrumbSchema = getBreadcrumbSchema([
    { name: "Home", url: `https://jarwater.com/${locale === "en" ? "" : locale}` },
    { name: tNav("services"), url: `${baseUrl}services/explainers` },
    { name: tNav("explainers"), url: `${baseUrl}services/explainers` },
  ]);
